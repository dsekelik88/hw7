...
firebase.auth().onAuthStateChanged(async function(user) {
  ...
  // Signed in
    ...
    // Pull movies from DB
    let apiKey = '149a32d09f374e3c2dfbd67ece962ed8'
    let response = await fetch(`https://api.themoviedb.org/3/movie/now_playing?api_key=${apiKey}&language=en-US`)
    let json = await response.json()
    let movies = json.results
    // console.log(movies)
    
    // // Pull filtered user data from Firestore
    // let querySnapshot = await db.collection('watched').where('userID', '==', userId).get()
    // let watchedMovies = querySnapshot.docs
    // console.log(watchedMovies)
    // console.log(`Number of movies ${userName} has watched = ${querySnapshot.size}`)
